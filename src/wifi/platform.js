export default {
  ...
  getStatus: "wpa_cli -iwlan0 status | sed -n -e '/^wpa_state=/{s/wpa_state=//;p;q}'",

  // A shell command that outputs the SSID of the current wifi network
  // or outputs nothing if we are not connected to wifi
  getConnectedNetwork: "wpa_cli -iwlan0 status | sed -n -e '/^ssid=/{s/ssid=//;p;q}'",
...
sed -e '/^$/d;/\\x00/d'`,
  ...
  startAP: 'sudo ifconfig wlan0 10.0.0.1 && sudo systemctl start hostapd && sudo systemctl start udhcpd',

  // Stop broadcasting an AP and attempt to reconnect to local wifi
  stopAP: 'sudo systemctl stop udhcpd && sudo systemctl stop hostapd && sudo ifconfig wlan0 0.0.0.0',
